...
class MasterJasaMusikController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data_jasa = $request->only('id_jenis_jasa', 'sk', 'deskripsi');

        if ($request->hasFile('gambar')) {
            $img = $request->file('gambar');
            $nama_img = time() . "-" . str_replace(' ', '_', $request->id_jenis_jasa) . "." . $img->getClientOriginalExtension();
    ...
    public function update(Request $request, string $id_jasa_musik)
    {
        ...
        if ($data) {
            ...
            if ($request->hasFile('gambar')) {
                ...
                $nama_img = time() . "-" . str_replace(' ', '_', $request->id_jenis_jasa) . "." . $extension;
                $img->move(public_path('/storage/img_upload'), $nama_img);
                $data->gambar = $nama_img;
            }

            // Memperbarui data jasa musik berdasarkan input yang valid
            $data->id_jenis_jasa = $request->input('id_jenis_jasa');
